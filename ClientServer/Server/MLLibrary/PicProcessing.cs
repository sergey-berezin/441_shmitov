...
namespace ClassLib
{
    public class PicProcessing
    {
        ...
        public IEnumerable<PictureResults> ObjectDetecting(string ImFolder)
        {
            ...
            List<Task<PictureResults>> tasks = new();

            foreach (string imageName in files)
            {
                if (!cancellationSource.Token.IsCancellationRequested)
                {
                    var one_image = OnePictureProcessing(new Bitmap(Image.FromFile(imageName)), imageName);
                    tasks.Add(one_image);
                }
                else
                {
            ...
            while (tasks.Count > 0 && !cancellationSource.IsCancellationRequested)
            {
                ...
                yield return tasks[taskId].Result;
                tasks.RemoveAt(taskId);
                files.RemoveAt(taskId);
            }
        }

        public async Task<PictureResults> OnePictureProcessing(Bitmap bitmap, string imageName)
        {
            _model = ModelCreation();
            return await Task.Factory.StartNew(() =>
            {
                ...
                var predictionEngine = mlContext.Model.CreatePredictionEngine<YoloV4BitmapData, YoloV4Prediction>(_model);
                
                // predict
                var predict = predictionEngine.Predict(new YoloV4BitmapData() { Image = bitmap });
                var results = predict.GetResults(classesNames, 0.3f, 0.7f);
                var pictureResults = new PictureResults(results, imageName);
